...
def carregar_tabela_fce():
    try:
        if os.path.exists("fce.csv"):
            df = pd.read_csv("fce.csv", sep=';', encoding='utf-8')
            # Verifica se o arquivo tem as colunas necessárias
            if 'Função' in df.columns and 'Valor' in df.columns:
                return df
            else:
                st.warning("Arquivo fce.csv não possui as colunas 'Função' e 'Valor'")
                return None
        return None
    except Exception as e:
...
Considerando que todas as remunerações de Funções Comissionadas estão reguladas pelos seguintes regramentos jurídicos: Lei nº 11.356/2006, Lei nº 11.526/2007, Lei nº 14.204/2021 e Lei nº 15.141/2025""")
        ...
        with col_info:
            if df_fce_ref is not None:
                ...
                st.error("Arquivo fce.csv não carregado. Função comissionada indisponível.")
    ...
    st.error("Não foi possível carregar os dados das tabelas. Verifique se os arquivos CSV estão presentes no diretório.")
